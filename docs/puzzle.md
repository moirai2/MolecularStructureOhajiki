...
> **「離して配置」の物理配置は自由**: おはじきを間に置くのは接続状態を視覚的に示すためです。おはじきの並べ方・間隔・向きに厳密なルールはありません。ただし**3×3の隣接関係（どのカードが隣か）は維持**してください。接地と離して配置が混在しても、全体を見やすく配置して構いません。

**制約: 接続に使える元素は、両カードに1個以上含まれているものに限ります。**

> **例**: 酸素（O）を含まないカード同士のペアでは、O接続は選べません。どちらか片方にしか含まれない元素も使用できません。
