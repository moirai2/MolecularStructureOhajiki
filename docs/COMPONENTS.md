...
> **ゲームごとの用途の違い:** MolOhajiki・MolSolitaire・MolUnion では各プレイヤーに5枚ずつ配布してプレイヤーボードとして使用。MolSurvival ではグリッド上下に共有配置しておはじきの発射エリアとして使用。**MolSugo では双六ループのマス（盤面の一部）として10枚全て使用**し、プレイヤーへの配布は行わない。TicTacMol では5種中炭素・水素・酸素の3種6枚のみ使用する。
...
**MolOhajiki本編** では合計15pt以上になった時点でラウンド終了の合図になります。
...
- **タイトル/QRカード**: 1枚（表: タイトル＋クレジット、裏: QRコード→詳細ルール）
  ...
  - MolUnion: グリッド上辺中央におはじきを弾くための台座として使用
  - MolSugo: 双六ループの「スタートマス」として使用
...
**MolSugo（モルスゴ）を遊ぶ場合のみ必要**です。他のゲームでは使いません。

**① 5色サイコロ（リソース用）— プレイヤー人数分**
- 各プレイヤーが5個持つ（黒・白・赤・青・黄 = 炭素・水素・酸素・窒素・その他）
- 目の数（1〜6）が各元素の現在の保有数を表すカウンター
- **個数**: 5個×人数分（3〜6人プレイ：15〜30個）
- **推奨**: ダイソーの「カラーサイコロ」（110円）
  - 商品ページ: https://jp.daisonet.com/products/4968583474228

**② 白サイコロ（移動用）— 2個（全員共有）**
- 手番プレイヤーが2個振り、合計値（2〜12）だけコマを進める
- 5色サイコロの白（水素）と区別するため、サイコロトレイや小皿の上に置いておくと管理しやすい

> 💡 ダイソーの「4色サイコロセット」＋「白サイコロセット」で揃います（緑を黒の代用として使用）。
