...
おはじき操作なしの双六型エンジンビルディング対戦です。**炭素・水素・酸素・窒素・その他**の5カテゴリで「いっせいのせい」によるマジョリティ争いを行い、**より多くのカテゴリで1位を取ったプレイヤーが勝利**します。fg pt（官能基ポイント）合計15以上はゲーム終了のトリガーであり、勝利点ではありません。
...
- タイトルカード 1枚（スタートマス）
- **5色サイコロ1セット × プレイヤー人数**（黒・白・赤・青・黄 = 炭素・水素・酸素・窒素・その他のリソースカウンター）
- **白サイコロ 2個**（移動用）
- **サイコロトレイ 1個**（円形配置を乱さないため。小皿等で代用可）
- **プレイヤーマーカー × プレイヤー人数**（おはじきやミープル等）
- **プレイヤーマット × プレイヤー人数**（GitHubで配布・各自A4ランドスケープで印刷。5色サイコロ置き場 + 元素枠5列）— [SVG](./molsugo_playerboard.svg)
...
2. **山札を作る**：シャッフルした3グループを積み重ね、最後に**タイトルカードを一番上**に置く
   ```
   [一番上]  タイトルカード（1枚）← 最後に乗せる
              Lv1グループ（20枚）
              Lv2グループ（20枚）
   [一番下]  Lv3+元素グループ（20枚）
   ```
...
3. **山札の上から1枚ずつ取り、円を描くようにテーブルに並べる**
   ...
   - 2枚目以降は時計回りに隣接させて並べる
   - **カードの上端1cmほどだけが見える程度に少しずつ重ねて並べる**と省スペースで円が作れる（上端には分子式が書かれているのでマスとして機能する）。マーカーを置ける程度の面積は残してください
...
プレイヤーマットは各自A4ランドスケープで印刷してご利用ください（[SVG](./molsugo_playerboard.svg)）。

![MolSugo プレイヤーマット](./molsugo_playerboard.png)
...
**ループ再構成:**
> 分子構造カードが購入されると、そのカードを取り除き、**時計回りで後続のカードを詰めて円を再構成**する。空きマスは発生しない。元素カードは盤に残り、消費されない。

**元素枠（アクティブカード）:**
> プレイヤーマット上の5つの色レーン（黒・白・赤・青・黄）。**各枠に最大1枚**の分子構造カードを置ける（最大5枚）。ここに置いたカードは軽減効果を発動する。
...
> **軽減は購入時の支払いにのみ適用されます。マジョリティ判定では、カードに書かれた分子式そのものを見ます。**
...
3. 止まったマスのカード種類に応じて処理：
   - 分子構造カード → 購入する OR リソース回復する（どちらか選択）
     ※購入できない場合（リソース不足・現在のリソースでは支払えない場合を含む）、
       または購入しない場合は、必ず回復扱いで手番を終える
   - 元素カード     → 該当元素を6まで回復（カードは盤に残す）
   - タイトルカード → 全元素リソース+1（最大6）。「着地」のみ処理し、通過の加算はしない
...
- **0の表現**: サイコロの目は1〜6のため0を表せない。リソースが0になったサイコロは**プレイヤーマットから取り除く**（取り除かれた状態 = 0）。回復時は目を1にセットして戻す。
...
- **着地時**: 全元素リソース +1（最大6）
- **通過時**: 全元素リソース +1（最大6）
- 着地と通過は別処理。同じ手番で両方は発生しない（着地した場合は通過加算なし）
- **購入不可**
...
4. このタイミングで**手元のカード ⇔ 元素枠 のスワップ**も可能（手元から1枚を元素枠に入れ、押し出された枠のカードを手元へ。**購入した手番のみ有効・回数制限なし**）。**購入したカードもスワップの対象にできる**（購入直後に元素枠→手元へ移動させることも可）
...
- **時計回りで後続のカードを詰めて円を再構成**（隙間ができないように）
- 購入されたカード上にいたマーカー（購入者・同乗者とも）は、**購入されたカードの1つ手前のカード**に移動する。

---

## ゲーム終了

**いずれかのプレイヤーの保有カード（元素枠＋手元）の fg pt 合計が 15 以上になったら、そのカードを購入した時点で達成者本人は必ず「終了宣言」を行う**。

- 達成者を含むそのラウンドを完走（**親の1つ手前のプレイヤー**まで全員が手番を行う）
- **達成者が親本人の場合**は、残りの全プレイヤーが手番を行ってから終了する
- 全員の手番数が揃ったところで終了処理に入る

> **例（4人プレイ、手番順A→B→C→D）**: 誰が宣言しても、Dの手番が終わってから終了。Dが宣言した場合はその場で終了。

> 購入可能な分子構造カードが盤上からなくなった場合も、そのラウンド完走で終了。

---

## マジョリティ判定（いっせいのせい）

ゲーム終了後、5つの元素カテゴリを**炭素 → 水素 → 窒素 → 酸素 → その他**の順に判定します。

> **各プレイヤーの手持ちカード = ゲーム中に積み上げた元素枠（最大5枚）＋手元のカード（保有カード）の全て**。この全カードがマジョリティ勝負のリソースです。各カテゴリで使用したカードは**使い切り**——元素枠から出したカードも手元から出したカードも、以降のカテゴリには使えません。どのカテゴリに何を出すかの配分が重要です。

### 手順

1. **炭素 → 水素 → 窒素 → 酸素 → その他**の順で1カテゴリずつ処理する
2. 各プレイヤーは**保有する全カード（元素枠5枚 + 手元のカード）から1枚を選んで伏せて出す**（出さなくてもよい。出さなかったプレイヤーはそのカテゴリでは勝利できないが、カードを温存できる）
3. 全員が選び終わったら **掛け声とともに同時に表向き**（掛け声は何でも可）
4. **そのカテゴリの元素を最も多く含むカード**を出した人がカテゴリ勝利 → **マジョリティポイント 1点獲得**
5. 同数の場合は**カードの fg pt が高い方**が勝利、それも同じなら**ドロー（誰も得点しない）**
6. 使ったカードは脇にどけ（ゲームから除外。元素枠・手元どちらのカードも以降は使えない）、次のカテゴリへ進む

### 最終タイブレーク

全カテゴリ終了後、マジョリティポイント同数の場合は、**判定で使用したカード（脇にどけたカード）の fg pt 総和が大きい人**が勝ち。それも同じなら**同率勝利**。
