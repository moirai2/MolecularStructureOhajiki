...
3. Lv1カードをシャッフルし、9枚を **3×3グリッド** に表向きで配置する（向きは自由）
4. グリッドの**上下に元素カード3枚（炭素・水素・酸素）ずつ**を**横向き**で並べる（発射台とグリッドの間の緩衝帯。乗っても盤外扱い。3枚の幅がグリッドと揃う。上下それぞれのプレイヤーから読みやすい向きに揃える）
...
> 各プレイヤーが**1個ずつ**おはじきを弾く → 押さえている状態を更新（= 自分のおはじきが乗っているカードを押さえる）→ 三列揃い判定 → カード獲得・補充
...
4. おはじきが**完全に止まる**まで待つ
5. **着地判定**: おはじきが**盤面（3×3の分子構造カード）に乗っていれば盤上**。元素カード上・マット上・テーブル上は全て**盤外**（元素カードはマットから浮いているが盤面外のため盤外扱い）。盤外のおはじきは手元に戻す（横から見て判断）
6. **三列揃い判定**（下記参照）
7. カード獲得・補充
...
ただし**三列揃い判定では 1個のおはじき = 1枚のカード分の押さえまでしかカウントされない**。そのため、判定の瞬間にプレイヤーが「このおはじきはどのカードに割り当てるか」を選んで確定する（観測するまでどのカードか決まらない、という意味で量子力学モデルと呼んでいます）。

> **例**: 中央と右のカードにまたがった場合——どちらを「押さえている」かは判定まで未定。三列揃い判定のとき、右のカードを押さえていれば列が完成する状況なら右を選べる。中央を選んだほうが有利なら中央を選んでよい。どちらを選ぶかはプレイヤーの自由。
...
> 🔄 **完成判定は両プレイヤー同時に行う**。すべての完成ラインを両者まとめて確認した後、カード獲得処理を行う。
...
どこを変えても自由です。アイデアはXで `#TicTacMol` を付けて教えてください。
