...
3. **元素カード**: 元素カード10枚を、グリッドの**上下に5枚ずつ**配置する（炭素・水素・酸素・窒素・その他、並び順は自由）。**各プレイヤー側の元素カードは、自分から読める向きに揃える**（上側プレイヤーは上側の5枚が自分から正しく読めるように、下側プレイヤーは下側の5枚を自分から読める向きに）。これがプレイヤーの**発射エリア**です（盤面ではなく境界外）
...
- **着地判定:** おはじきが**盤面（中央5×3の分子構造カード）に乗っていれば盤上**。元素カード上・マット上・テーブル上は全て**盤外**（元素カードはマットから浮いているが盤面外のため盤外扱い。横から見て判断）
- **複数の分子構造カードにまたがった場合（量子力学モデル）:** 物理的に複数のカードに触れている場合、**触れているカードすべてに乗っている**扱いになる（上手い人はあえてまたがるよう狙うこともできる。競合の回避や最多元素の回避に使える）。どのカードに割り当てるかは事前には決まっておらず、**生存判定の瞬間にプレイヤーが1枚を選んで着地カードを確定**する（=観測で初めて決まる）。**小さなお子さんとプレイする場合は「最も多く乗っているカード」を着地カードとする裁定にすると揉めにくい**

### ② 生存判定フェーズ

以下の3つの判定を **盤外→競合→最多元素 の順に確認**し、**いずれかが該当した時点でラウンド終了**。後のフェーズは判定されません。該当したプレイヤーがおはじきを失い、**他全員は弾いたおはじきを手元に戻します**。

| 判定 | 該当条件 | 処理 |
|------|---------|------|
| **盤外** | 盤面外（元素カード上・マット上・テーブル上）に出したプレイヤーが**1人でもいる** | 盤外プレイヤー全員がおはじき1個失う。**他全員は手元に戻す**。判定終了 |
| **競合** | 盤外該当者なし、かつ同じ分子構造カードに2人以上が着地している | 競合カードに乗ったプレイヤー全員がおはじき1個失う。**他全員は手元に戻す**。判定終了 |
| **最多元素** | 盤外・競合該当者なし（全員別々の分子構造カードに着地） | **着地した分子構造カードの中で**元素合計（total）が最大のカードに乗ったプレイヤーがおはじき1個失う（同数最多は該当者全員） |
...
>   - 盤外: Aが盤外 → **A だけ1個失う**。BCD はおはじきが手元に戻る（競合・最多元素は判定しない）
> - **例2（競合発動で終了）**: 盤外なし。ABがH₂Oに着地、Cが C₆H₆、Dが CH₄
>   - 盤外: 該当なし → 競合: ABがH₂Oで競合 → **AB だけ1個失う**。CD はおはじきが手元に戻る（最多元素は判定しない）
> - **例3（最多元素のみ発動）**: 全員違う分子構造カードに着地、最大totalがC₆H₆（total=12）でAが乗っている
>   - 盤外・競合: 該当なし → 最多元素: A が最多 → **Aだけ1個失う**
> - **例4（同数最多発動）**: 4人全員違う分子構造カードで全て同じtotal
...
どこを変えても自由です。アイデアはXで `#MolSurvival` を付けて教えてください。
